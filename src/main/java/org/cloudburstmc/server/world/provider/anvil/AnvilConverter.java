...
import org.cloudburstmc.server.world.chunk.*;
import org.cloudburstmc.server.world.provider.LegacyBlockConverter;
import org.cloudburstmc.server.world.provider.anvil.palette.BiomePalette;
...
public class AnvilConverter {
    ...
    public static void convertToCloudburst(ChunkBuilder chunkBuilder, ByteBuf chunkBuf) throws IOException {
        ...
        try (ByteBufInputStream stream = new ByteBufInputStream(chunkBuf);
            ...
            if (!tag.containsKey("World") || !(tag.get("World") instanceof NbtMap)) {
                throw new IllegalArgumentException("No world tag found in chunk data");
            }
            tag = tag.getCompound("World");
        }
...
//                        entryTag.getInt("p"), false));
    ...
    private static Location getLocation(NbtMap tag, Chunk chunk) {
        ...
        return Location.from(position, yaw, pitch, chunk.getWorld());
    }
