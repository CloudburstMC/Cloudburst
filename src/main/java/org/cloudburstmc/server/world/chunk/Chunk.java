...
import org.cloudburstmc.server.world.BlockUpdate;
import org.cloudburstmc.server.world.ChunkLoader;
import org.cloudburstmc.server.world.World;
import org.cloudburstmc.server.world.chunk.bitarray.BitArrayVersion;
...
public final class Chunk implements IChunk, Closeable {
    ...
    public Chunk(int x, int z, World world) {
        this(new UnsafeChunk(x, z, world));
    }
    ...
    public void init() {
        ...
        if (init) {
            try (Timing ignored = unsafe.getWorld().timings.syncChunkLoadEntitiesTimer.startTiming()) {
                boolean dirty = false;
            ...
            for (BlockUpdate update : blockUpdates) {
                this.unsafe.getWorld().scheduleUpdate(update);
            }
