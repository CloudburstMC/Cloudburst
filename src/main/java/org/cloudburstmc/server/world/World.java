...
import org.cloudburstmc.server.world.chunk.ChunkSection;
...
import org.cloudburstmc.server.world.manager.LevelChunkManager;
import org.cloudburstmc.server.world.particle.DestroyBlockParticle;
 ...
 */
@Log4j2
public class World implements ChunkManager, Metadatable {
...
//    private final List<BlockUpdateEntry> nextTickUpdates = Lists.newArrayList();
    ...
    World(Server server, String id, WorldProvider worldProvider, WorldData worldData) {
        this.id = id;
...
//                this.chunkProvider = chunkProvider.getConstructor(World.class, String.class).newInstance(this, newPath);
//            } else {
//                this.chunkProvider = chunkProvider.getConstructor(World.class, String.class).newInstance(this, path);
//            }
//        } catch (Exception e) {
...
//
//        if (fullConvert) {
//            this.server.getLogger().info(this.server.getLanguage().translate("cloudburst.world.updating",
...
//        }
        ...
        log.info(this.server.getLanguage().translate("cloudburst.world.preparing",
                TextFormat.GREEN + getId() + TextFormat.WHITE));

        this.generator = GeneratorRegistry.get().getGeneratorFactory(this.worldData.getGenerator()).create(this.getSeed(), this.worldData.getGeneratorOptions());

        if (this.worldData.getRainTime() <= 0) {
            setRainTime(ThreadLocalRandom.current().nextInt(168000) + 12000);
        }

        if (this.worldData.getLightningTime() <= 0) {
            setThunderTime(ThreadLocalRandom.current().nextInt(168000) + 12000);
        }

        this.updateQueue = new BlockUpdateScheduler(this, this.worldData.getCurrentTick());
    ...
    final public WorldProvider getProvider() {
    ...
    public void close() {
        ...
        try {
            ...
            throw new LevelException("Error occurred whilst closing world", e);
        }
        this.provider = null;
    ...
    public void addParticleEffect(Vector3f pos, Identifier identifier) {
        this.addParticleEffect(pos, identifier, -1, this.worldData.getDimension(), (Player[]) null);
    }

    public void addParticleEffect(Vector3f pos, Identifier identifier, long uniqueEntityId) {
        this.addParticleEffect(pos, identifier, uniqueEntityId, this.worldData.getDimension(), (Player[]) null);
    }
    ...
    public boolean unload(boolean force) {
        WorldUnloadEvent ev = new WorldUnloadEvent(this);

        if (this == this.server.getDefaultWorld() && !force) {
            ev.setCancelled();
        }
        ...
        log.info(this.server.getLanguage().translate("cloudburst.world.unloading",
        ...
        for (Player player : new ArrayList<>(this.getPlayers().values())) {
            if (this == defaultWorld || defaultWorld == null) {
                player.close(player.getLeaveMessage(), "Forced default world unload");
            } else {
                player.teleport(this.server.getDefaultWorld().getSafeSpawn());
            }
        }

        if (this == defaultWorld) {
            this.server.setDefaultWorld(null);
        }
    ...
    public void checkTime() {
        this.worldData.checkTime(this.tickRate);
    }
    ...
    public void doTick(int currentTick) {
        try (Timing ignored = this.timings.doTick.startTiming()) {
            ...
            // Tick Weather
            if (this.getDimension() != World.DIMENSION_NETHER && this.getDimension() != World.DIMENSION_THE_END && this.doWeatherCycle()) {
                this.worldData.setRainTime(getRainTime() - 1);
                if (this.worldData.getRainTime() <= 0) {
                    if (!this.setRaining(this.worldData.getRainLevel() <= 0)) {
                        if (this.worldData.getRainLevel() > 0) {
                ...
                this.worldData.setLightningTime(this.worldData.getLightningTime() - 1);
                if (this.worldData.getLightningTime() <= 0) {
                    if (!this.setThundering(this.worldData.getLightningLevel() <= 0)) {
                        if (this.worldData.getLightningLevel() > 0) {
            ...
            try (Timing ignored2 = this.timings.tickChunks.startTiming()) {
                ...
                if (this.worldData.getGameRules().isDirty()) {
                    GameRulesChangedPacket packet = new GameRulesChangedPacket();
                    this.worldData.getGameRules().toNetwork(packet.getGameRules());
                    Server.broadcastPacket(players.values().toArray(new Player[0]), packet);
                    this.worldData.getGameRules().refresh();
                }
            }
        }
    }
    ...
    public void checkSleep() {
        ...
        if (resetTime) {
            int time = this.getTime() % World.TIME_FULL;
    ...
    private boolean save(boolean force, boolean sync) {
        ...
        this.server.getEventManager().fire(new WorldSaveEvent(this));

        CompletableFuture<Void> chunksFuture = this.saveChunks();
        CompletableFuture<Void> dataFuture = this.provider.saveLevelData(this.worldData);
        ...
        if (player != null) {
            ...
            if (!ev.isCancelled()) {
                ...
                if (item.getId() == ItemIds.BUCKET && ItemBucket.getBlockIdFromDamage(item.getMeta()) == BlockIds.FLOWING_WATER) {
                    player.getWorld().sendBlocks(new Player[]{player}, new Block[]{new CloudBlock(this, block.getPosition(), new BlockState[]{BlockStates.AIR, BlockStates.AIR})}, UpdateBlockPacket.FLAG_ALL_PRIORITY);
                }
                return null;
            }
    ...
    public void addBlockEntity(BlockEntity blockEntity) {
        if (blockEntity.getWorld() != this) {
            throw new LevelException("Invalid Block Entity world");
        }
    ...
    public void scheduleBlockEntityUpdate(BlockEntity entity) {
        checkNotNull(entity, "entity");
        Preconditions.checkArgument(entity.getWorld() == this, "BlockEntity is not in this world");
    ...
    public boolean isSpawnChunk(int x, int z) {
        Vector3i spawn = this.worldData.getSpawn();
    ...
    public int getTime() {
        return (int) this.worldData.getTime();
    }
    ...
    public void setTime(int time) {
        this.worldData.setTime(time);
        this.sendTime();
    }

    public long getCurrentTick() {
        return this.worldData.getCurrentTick();
    }
    ...
    public long getSeed() {
        return this.worldData.getRandomSeed();
    }
    ...
    public boolean isRaining() {
        return this.worldData.getRainLevel() > 0;
    }
    ...
    public boolean setRaining(boolean raining) {
        ...
        this.worldData.setRainLevel(raining ? 1 : 0);
    ...
    public int getRainTime() {
        return this.worldData.getRainTime();
    }

    public void setRainTime(int rainTime) {
        this.worldData.setRainTime(rainTime);
    }

    public boolean isThundering() {
        return isRaining() && this.worldData.getLightningLevel() > 0;
    }
    ...
    public boolean setThundering(boolean thundering) {
        ...
        this.worldData.setLightningLevel(thundering ? 1 : 0);
    ...
    public int getThunderTime() {
        return this.worldData.getLightningTime();
    }

    public void setThunderTime(int thunderTime) {
        this.worldData.setLightningTime(thunderTime);
    }
    ...
    public int getDimension() {
        return this.worldData.getDimension();
    }
