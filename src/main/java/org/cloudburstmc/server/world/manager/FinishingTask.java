...
public final class FinishingTask implements BiFunction<Chunk, List<Chunk>, Chunk> {
    ...
    public Chunk apply(@NonNull Chunk chunk, List<Chunk> chunks) {
        ...
        PRandom random = new FastPRandom(chunk.getX() * 9050650275199519859L ^ chunk.getZ() * 5251710924988638743L ^ chunk.getWorld().getSeed());
        ...
        try {
            chunk.getWorld().getGenerator().finish(random, new PopulationChunkManager(chunk, lockableChunks, chunk.getWorld().getSeed()), chunk.getX(), chunk.getZ());
