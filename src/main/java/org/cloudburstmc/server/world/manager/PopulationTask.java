...
public final class PopulationTask implements BiFunction<Chunk, List<Chunk>, Chunk> {
    ...
    public Chunk apply(@NonNull Chunk chunk, List<Chunk> chunks) {
        ...
        PRandom random = new FastPRandom(chunk.getX() * 6169336838570288771L ^ chunk.getZ() * 1173358236373774883L ^ chunk.getWorld().getSeed());
        ...
        try {
            chunk.getWorld().getGenerator().populate(random, new PopulationChunkManager(chunk, lockableChunks, chunk.getWorld().getSeed()), chunk.getX(), chunk.getZ());
