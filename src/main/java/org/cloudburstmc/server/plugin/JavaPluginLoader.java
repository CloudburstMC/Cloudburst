...
public class JavaPluginLoader implements PluginLoader {
    ...
    public Plugin loadPlugin(File file) throws Exception {
        ...
        if (description != null) {
            log.info(this.server.getLanguage().translate("cloudburst.plugin.load", description.getFullName()));
    ...
    public PluginDescription getPluginDescription(File file) {
        try (JarFile jar = new JarFile(file)) {
            JarEntry entry = jar.getJarEntry("cloudburst.yml");
            if (entry == null) {
    ...
    public void enablePlugin(Plugin plugin) {
        if (plugin instanceof PluginBase && !plugin.isEnabled()) {
            log.info(this.server.getLanguage().translate("cloudburst.plugin.enable", plugin.getDescription().getFullName()));
    ...
    public void disablePlugin(Plugin plugin) {
        if (plugin instanceof PluginBase && plugin.isEnabled()) {
            log.info(this.server.getLanguage().translate("cloudburst.plugin.disable", plugin.getDescription().getFullName()));
