...
public class ParticleCommand extends Command {
    ...
    private Particle getParticle(String name, Location loc, int data) {
        ...
        switch (name) {
            ...
            case "snowballpoof":
                return new ItemBreakParticle(pos, ItemStack.get(ItemIds.SNOWBALL));
            case "slime":
                return new ItemBreakParticle(pos, ItemStack.get(ItemIds.SLIME_BALL));
            case "itembreak":
                if (data != -1 && data != 0) {
                    return new ItemBreakParticle(pos, ItemStack.get(data));
                }
                break;
            case "terrain":
                if (data != -1 && data != 0) {
//                    return new TerrainParticle(pos, BlockState.get(data)); //TODO: block name
        ...
        if (name.startsWith("iconcrack_")) {
            ...
            if (d.length == 3) {
                return new ItemBreakParticle(pos, ItemStack.get(Integer.parseInt(d[1]), Integer.parseInt(d[2])));
            }
