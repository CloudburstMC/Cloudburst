 ...
 * @author DaPorkchop_
 */
final class PopulatorDeserializer extends ValueDeserializer<Populator> {
    @Override
    public Populator deserialize(JsonParser p, DeserializationContext ctxt) throws JacksonException {
        ...
        try {
            return id == NextGenerationPass.ID
                    ? NextGenerationPass.INSTANCE : ctxt.readValue(p, StandardGeneratorRegistries.populator().get(id));
        } catch (Exception e) {
