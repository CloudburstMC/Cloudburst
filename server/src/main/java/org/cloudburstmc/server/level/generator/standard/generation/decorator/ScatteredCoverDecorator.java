    ...
    public void decorate(PRandom random, IChunk chunk, int x, int z) {
        ...
        for (int y = min(chunk.getHighestBlock(x, z), 254); y >= 0; y--) {
            if (random.nextDouble() < chance
                    && on.test(org.cloudburstmc.server.registry.BlockRegistry.get().getRuntimeId(chunk.getBlock(x, y, z, 0)))
                    && replace.test(org.cloudburstmc.server.registry.BlockRegistry.get().getRuntimeId(chunk.getBlock(x, y + 1, z, 0)))) {
