...
import static org.cloudburstmc.api.block.BlockTags.*;
import static org.cloudburstmc.api.block.BlockTypes.*;

@UtilityClass
class VanillaBlockTags {

    private static final Map<BlockTagKey, Set<BlockType>> DIRECT = new IdentityHashMap<>();
    private static final Map<BlockTagKey, Set<BlockTagKey>> INCLUDES = new IdentityHashMap<>();
    ...
    static {
        ...
        tag(LIGHTNING_RODS, LIGHTNING_ROD, EXPOSED_LIGHTNING_ROD, WEATHERED_LIGHTNING_ROD,
                OXIDIZED_LIGHTNING_ROD, WAXED_LIGHTNING_ROD, WAXED_EXPOSED_LIGHTNING_ROD,
                WAXED_WEATHERED_LIGHTNING_ROD, WAXED_OXIDIZED_LIGHTNING_ROD);

        tag(SUPPORTS_CHORUS_FLOWER, END_STONE);
        tag(SUPPORTS_CHORUS_PLANT, END_STONE);
        tag(SUPPORTS_VEGETATION, DIRT, COARSE_DIRT, DIRT_WITH_ROOTS, FARMLAND, GRASS_BLOCK, MOSS_BLOCK, MUD, MYCELIUM, PALE_MOSS_BLOCK, PODZOL);
        inherit(SUPPORTS_DRY_VEGETATION, SUPPORTS_VEGETATION);
        tag(SUPPORTS_DRY_VEGETATION, SAND, RED_SAND, HARDENED_CLAY,
                ...
                LIME_TERRACOTTA, MAGENTA_TERRACOTTA, ORANGE_TERRACOTTA, PINK_TERRACOTTA,
                PURPLE_TERRACOTTA, RED_TERRACOTTA, WHITE_TERRACOTTA, YELLOW_TERRACOTTA
        );
        inherit(SUPPORTS_WITHER_ROSE, SUPPORTS_VEGETATION);
        tag(SUPPORTS_WITHER_ROSE, NETHERRACK, SOUL_SAND, SOUL_SOIL);
    ...
    static void bind(BlockType type) {
        Set<BlockTagKey> tags = Collections.newSetFromMap(new IdentityHashMap<>());
        for (BlockTagKey key : keys()) {
            if (values(key).contains(type)) {
                tags.add(key);
            }
        }
    ...
    static void freeze() {
        for (BlockTagKey key : keys()) {
            RESOLVED.put(key, new ResolvedBlockTag(key, values(key)));
        }
    }

    private static Set<BlockTagKey> keys() {
        Set<BlockTagKey> keys = Collections.newSetFromMap(new IdentityHashMap<>());
        keys.addAll(DIRECT.keySet());
        keys.addAll(INCLUDES.keySet());
        return keys;
    }

    private static void tag(BlockTagKey tag, BlockType... types) {
        Collections.addAll(DIRECT.computeIfAbsent(tag,
                ignored -> Collections.newSetFromMap(new IdentityHashMap<>())), types);
    }

    private static void inherit(BlockTagKey target, BlockTagKey... sources) {
        Collections.addAll(INCLUDES.computeIfAbsent(target,
                ignored -> Collections.newSetFromMap(new IdentityHashMap<>())), sources);
    }

    static BlockTag resolve(BlockTagKey key) {
        BlockTag tag = RESOLVED.get(key);
        if (tag == null) {
            throw new IllegalArgumentException("Unknown block tag: " + key);
        }
        return tag;
    }

    static Set<BlockType> values(BlockTagKey tag) {
        Set<BlockType> values = Collections.newSetFromMap(new IdentityHashMap<>());
        collect(tag, Collections.newSetFromMap(new IdentityHashMap<>()), values);
        return Collections.unmodifiableSet(values);
    }

    private static void collect(BlockTagKey tag, Set<BlockTagKey> visited, Set<BlockType> values) {
        if (!visited.add(tag)) {
            return;
        }
        values.addAll(DIRECT.getOrDefault(tag, Set.of()));
        for (BlockTagKey included : INCLUDES.getOrDefault(tag, Set.of())) {
            collect(included, visited, values);
        }
    }

    private record ResolvedBlockTag(BlockTagKey key, Set<BlockType> values) implements BlockTag {
        @Override
        public BlockTagKey getKey() {
            return this.key;
        }

        @Override
        public boolean isTagged(BlockType type) {
