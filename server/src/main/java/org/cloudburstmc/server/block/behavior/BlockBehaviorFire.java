                                ...
                                if (x != block.getX() || y != block.getY() || z != block.getZ()) {
                                    ...
                                    if (chance > 0) {
                                        ...
                                        if (t > 0 && random.nextInt(k) <= t) {
                                            ...
                                            if (!e.isCancelled()) {
                                                blockState.set(BlockRegistry.get().getBlock(BlockTypes.FIRE).withTrait(BlockTraits.AGE, age_));
    ...
    private void tryToCatchBlockOnFire(Block fire, Block block, int bound, int damage) {
        ...
        if (random.nextInt(bound) < burnAbility) {
            ...
            if (random.nextInt(damage + 10) < 5) {
                ...
                if (!e.isCancelled()) {
                    block.set(BlockRegistry.get().getBlock(BlockTypes.FIRE).withTrait(BlockTraits.AGE, meta), true);
