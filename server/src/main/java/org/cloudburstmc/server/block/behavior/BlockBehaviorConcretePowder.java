...
public class BlockBehaviorConcretePowder extends BlockBehaviorFallable {
    ...
    public int onUpdate(Block block, int type) {
        if (type == Level.BLOCK_UPDATE_NORMAL) {
            ...
            for (int side = 1; side <= 5; side++) {
                BlockState blockState = this.getSide(BlockFace.fromIndex(side));
    ...
    @Override
    public boolean place(Item item, BlockState b, BlockState target, BlockFace face, Vector3f clickPos, Player player) {
